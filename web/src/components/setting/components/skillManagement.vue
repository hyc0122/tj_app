  ...
  <div class="skillManagement">
    <aside class="sidebarPanel">
      ...
      <div class="treeWrap">
        ...
        <t-empty v-else :description="diagnostic || $t('setting.skillManagement.empty')" />
      </div>
    </aside>
...
async function fetchList() {
  try {
    ...
    if (Array.isArray(data)) {
      entries.value = data;
      diagnostic.value = data.length ? "" : String(window.$t("setting.skillManagement.empty"));
      return;
    }
    entries.value = Array.isArray(data?.entries) ? data.entries : [];
    diagnostic.value = typeof data?.diagnostic === "string" ? data.diagnostic : "";
  } catch (e) {
    console.error(e);
    entries.value = [];
    diagnostic.value = "Skills 加载失败，请检查当前账号数据目录或重新登录后重试。";
  }
}
