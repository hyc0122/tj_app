<template>
  <div class="novel" ref="novelRef">
    ...
    <importNovel
      v-model="importNovelShow"
      :confirm-visible="confirmImportVisible"
      @select="() => void getNovel()"
    />
    <editNodel v-model="editNodelShow" :formData="formData" @select="() => void getNovel()" />
  </div>
</template>
...
} from "./components/novel-import-contract";
...
/**
 * 列表查询：与 addNovel 使用同一正安全整数 projectId。
 * 返回是否有可见行，供导入确认使用。
 */
async function getNovel(options?: { resetPage?: boolean; clearSearch?: boolean }): Promise<boolean> {
  if (options?.resetPage) pagination.value.page = 1;
  if (options?.clearSearch) searchText.value = "";
  loading.value = true;
  try {
    const projectId = normalizeNovelProjectIdInput(project.value?.id as unknown);
    const res = await axios.post("/novel/getNovel", {
      projectId,
      page: pagination.value.page,
      limit: pagination.value.pageSize,
      search: searchText.value,
    });
    const page = parseNovelListResponse(res);
    tableData.value = page.rows as OriginalText[];
    pagination.value.total = page.total;
    return novelListHasVisibleRows(page);
  } catch (e) {
    tableData.value = [];
    pagination.value.total = 0;
    const msg =
      typeof (e as { message?: unknown })?.message === "string"
        && (e as { message: string }).message !== "[object Object]"
        ? (e as { message: string }).message
        : $t("workbench.novel.msg.loadFailed");
...
/** 导入完成后：重置到第一页、清理搜索，await 刷新并确认可见。 */
async function confirmImportVisible(): Promise<boolean> {
  const hadSearch = Boolean(searchText.value?.trim());
  const visible = await getNovel({ resetPage: true, clearSearch: true });
  if (!visible && hadSearch) {
    // 已清搜索仍不可见时由调用方统一处理；此处仅保证筛选不会隐藏新行。
  }
  return visible;
}
...
function startEventAnalysis() {
  const dialog = DialogPlugin.confirm({
    ...
    onConfirm: () => {
      ...
      try {
        projectId = normalizeNovelProjectIdInput(project.value?.id as unknown);
      } catch {
        window.$message.error($t("workbench.novel.import.msg.invalidProject") as string);
        return;
      }
      axios
