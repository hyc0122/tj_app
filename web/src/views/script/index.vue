  ...
  <div class="script">
    ...
    <div class="contentArea">
      ...
      <div v-else class="scriptsList f w">
        <div v-for="(item, index) in scripts" :key="index" @click="handleScriptClick(item)" class="scriptCard module-interactive" tabindex="0" @keydown.enter.prevent="handleScriptClick(item)">
          <!-- 仅外层 scriptCard 负责 transform，内层 t-card 不再叠加 module-interactive -->
...
// 搜索剧本：仅在有效活动项目下发起；离开工作区后的陈旧调用直接跳过。
async function searchScripts() {
  let projectId: number;
  try {
    // 中文注释：统一本地项目边界，禁止 Number/trim 旁路
  ...
  try {
    ...
    // 预期的项目关闭 404 不重复 toast；真实用户操作失败仍提示。
    if (error?.__staleProjectMiss) return;
