        ...
        <div class="box pr">
          ...
          <t-chat-sender
            ...
            <template #footer-prefix>
              <!-- attach=body 避免父容器 overflow 裁剪；仍走 Popper，禁止绝对 top/left 补丁 -->
              <t-popup trigger="click" placement="top-left" :attach="popupAttach" destroy-on-close>
                <t-button
                  shape="square"
                  variant="outline"
                  size="small"
                  :disabled="status === 'pending' || status === 'streaming'"
                  :title="$t('workbench.scriptAgent.settings')"
                >
                  <template #icon>
                ...
                <template #content>
                  <div class="settingMenu script-agent-setting-menu">
                    <div class="settingMenuItem" @click="handleReconnect()">
                      <i-api size="14" />
                      <span>{{ $t("workbench.scriptAgent.reconnect") }}</span>
                    </div>
                    <t-tooltip :content="writeDisabledReason" :disabled="canWriteMemory">
                      <div
                        class="settingMenuItem"
                        :class="{ 'is-disabled': !canWriteMemory }"
                        @click="canWriteMemory && handleClearMemory('message')"
                      >
                        <i-delete size="14" />
                    ...
                    <t-tooltip :content="writeDisabledReason" :disabled="canWriteMemory">
                      <div
                        class="settingMenuItem"
                        :class="{ 'is-disabled': !canWriteMemory }"
                        @click="canWriteMemory && handleClearMemory('summary')"
                      >
                        <i-close size="14" />
                    ...
                    <t-tooltip :content="writeDisabledReason" :disabled="canWriteMemory">
                      <div
                        ...
                        :class="{ 'is-disabled': !canWriteMemory }"
                        @click="canWriteMemory && handleClearMemory('all')"
                      >
...
import {
  requestClearScriptAgentMemoryIfAllowed,
} from "@/features/tianjiang/script-agent/write-access";
import { toLocalProjectId } from "@/features/tianjiang/project/local-project-id";
// 活动项目与访问权限同一 store；写权限只用 canWrite/access，不用 Project 业务字段
const activeProjectStore = projectStore();
const { project, canWrite: canWriteMemory } = storeToRefs(activeProjectStore);

/** 当前活动项目的本地数字 ID（HTTP body 用） */
function currentLocalProjectId(): number {
  return toLocalProjectId(project.value?.id);
}
...
// 弹层挂到 body，避免 Splitpanes/overflow 裁剪
const popupAttach = () => document.body;
const writeDisabledReason = computed(() =>
  canWriteMemory.value ? "" : $t("workbench.scriptAgent.msg.readonlyNoClear"),
);
...
onMounted(() => {
  ...
  getPlanData();
  getNovel();
  // 每次进入页面建立新连接，不复用已失效 Socket 状态
  scriptAgentStore().connect();

  if (messages.value.length <= 1) getHistory();
});

// 离开剧本 Agent：断开本页 Socket 并取消在途生成；不得影响账号级 syncCoordinator
onBeforeUnmount(() => {
  try {
...
async function getPlanData() {
  // 边界：projectId 必须为 JSON number；保存统一 setPlanData，无需本地 agentWorkDataId
  const { data } = await axios.post("/scriptAgent/getPlanData", {
    projectId: currentLocalProjectId(),
...
function handleClearMemory(type: "message" | "summary" | "all" | "reconnect") {
  // 双重门禁：模板禁用 + 确认回调内再检查 canWrite，只读/recovery 绝不能发 clearMemory
  if (!canWriteMemory.value) {
    window.$message?.warning?.($t("workbench.scriptAgent.msg.readonlyNoClear"));
    return;
  }
  const dialog = DialogPlugin.confirm({
    ...
    theme: "warning",
    onConfirm: async () => {
      const outcome = await requestClearScriptAgentMemoryIfAllowed(canWriteMemory.value, () =>
        axios.post(`/agents/clearMemory`, {
          projectId: currentLocalProjectId(),
          agentType: "scriptAgent",
          type,
        }),
...
function handleReconnect() {
  const dialog = DialogPlugin.confirm({
    ...
    theme: "warning",
    onConfirm: async () => {
      // 必须调用剧本 Agent 自己的 reconnect，禁止误调 productionAgent
      scriptAgentStore().reconnect();
      dialog.destroy();
    },
  });
}
...
function getNovel() {
  axios.post("/novel/getNovelData", { projectId: currentLocalProjectId() }).then(({ data }: any) => {
...
function onConfirm(value: string) {
  // 统一更新本地 planData 后走 setPlanData（事务 upsert）
  if (currentTable.value == 1) planData.value.storySkeleton = value;
  else if (currentTable.value == 2) planData.value.adaptationStrategy = value;
  scriptAgentStore()
    .setPlanData()
    .then(() => {
