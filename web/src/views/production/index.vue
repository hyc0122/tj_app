  ...
  <VueFlow
    ...
    <div class="floatingWindow">
      <div class="episodesSelect f ac">
        ...
        <t-tooltip placement="bottom" theme="primary" :content="$t('workbench.production.guideReplay')">
          <t-button
            class="guide-replay-btn"
            :aria-label="$t('workbench.production.guideReplay')"
            @click="replayProductionGuideOnce"
            variant="outline"
            style="margin-left: 8px">
            <template #icon>
              <i-help size="16" />
            </template>
            {{ $t("workbench.production.guideReplay") }}
          </t-button>
        </t-tooltip>
    ...
    <ProductionGuideControls
      v-model="current"
      :steps="steps"
      :complete="completeProductionGuide" />
...
import { createProductionGuideController } from "./production-guide";
import ProductionGuideControls from "./components/ProductionGuideControls.vue";
...
const productionGuide = createProductionGuideController(axios);
const current = productionGuide.current;
const replayProductionGuideOnce = productionGuide.replayOnce;

async function completeProductionGuide(): Promise<boolean> {
  const completed = await productionGuide.complete();
  if (!completed) {
    // 中文注释：只展示控制器提供的固定安全文案，禁止透传网络、路径或存储层异常。
    window.$message.error(productionGuide.errorMessage.value);
  }
  return completed;
}

onMounted(() => {
  void productionGuide.initialize();
});
const steps = [
  {
