 ...
 * 其他尚未迁移的 production 阶段暂时保留既有保存行为。
 */
export function shouldPersistProductionXmlTag(tag: string, status: string): boolean {
  return status === "complete" && tag !== "scriptPlan";
}
  ...
  return defineStore(`productionAgent-${projectId}`, () => {
    ...
    const { connected, messages, chat, stopGenerate, socket, status, reconnect, connect, disconnect } = useChat({
      ...
      auth: () => {
        ...
        const scriptId =
          episodesId.value == null ? undefined : toPositiveSafeInteger(episodesId.value);
        return {
          // isolationKey 仅展示性；服务端必须自行派生并忽略客户端任意值
          isolationKey: scriptId
            ? `${id}:productionAgent:${scriptId}`
            : `${id}:productionAgent`,
          projectId: id,
          scriptId,
        };
      ...
      onXmlTag: async (data) => {
        ...
        //   }
        // }
        if (shouldPersistProductionXmlTag(tag, status)) {
          throttledFn();
        }
      },
    });
    ...
    watch(
      socket,
      (s, _previous, onCleanup) => {
        if (s) {
          s.on("connect", () => {
            getHistory();
          });
          const onArtifactCommitted = async (event: {
            stage?: string;
            projectId?: unknown;
            episodesId?: unknown;
          }) => {
            if (event?.stage !== "directorPlan") return;
            const currentProjectId = toLocalProjectId(projectId);
            const currentEpisodesId = toPositiveSafeInteger(episodesId.value);
            if (
              event.projectId !== currentProjectId ||
              event.episodesId !== currentEpisodesId
            ) {
              return;
            }
            // 中文注释：以后端事务结果覆盖流式预览，禁止调用 setFlowData 反向写回。
            await getFlowData();
          };
          s.on("artifactCommitted", onArtifactCommitted);
          onCleanup(() => {
            s.off("artifactCommitted", onArtifactCommitted);
          });
    ...
    async function setFlowData(scriptId?: number) {
      const episodes =
        scriptId != null ? toPositiveSafeInteger(scriptId) : toPositiveSafeInteger(episodesId.value);
      await axios.post(
    ...
    async function getFlowData() {
      const { data } = await axios.post(
        ...
        localProjectBody(projectId, {
          episodesId: toPositiveSafeInteger(episodesId.value),
        }),
      );
      flowData.value = data;
    }
    ...
    function updateContext() {
      if (episodesId.value == null || episodesId.value < 0) return;
      const id = toLocalProjectId(projectId);
      const scriptId = toPositiveSafeInteger(episodesId.value);
      // 中文注释：必须发送 number；服务端忽略客户端 isolationKey 并自行派生
      const ctx = {
        isolationKey: `${id}:productionAgent:${scriptId}`,
        projectId: id,
        scriptId,
      };
