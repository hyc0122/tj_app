...
export function useSocket<T extends SocketEventMap = SocketEventMap>(url = "http://127.0.0.1:10588", authOptions?: Record<string, any>) {
  ...
  const connect = () => {
    ...
    socket = io(url, {
      // 与 App Engine.IO path 对齐，确保 Path=/api 的会话 Cookie 被发送。
      path: ENGINE_IO_PATH,
      ...
      timeout: 10000,
      // Socket.IO 同源握手自动携带 HttpOnly 会话 Cookie。
      auth: { ...authOptions },
    });
