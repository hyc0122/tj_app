...
import { ENGINE_IO_PATH } from "@/utils/socket-path";
...
export function useChat(options: UseChatOptions) {
  ...
  /** 延迟 reconnect 定时器：disconnect/卸载时必须取消，防止离开页面后旧项目回连 */
  ...
  const parseXmlTag = (text: string, tag: string) => {
    ...
    const tokenRegex = new RegExp(`<(/?)${escapedTag}(\\s[^>]*)?>`, "g");
    ...
    while ((token = tokenRegex.exec(text)) !== null) {
      if (token[1] === "/") {
        ...
        if (open) completeBlocks.push({ open, closeIndex: token.index });
        continue;
      }
      openStack.push({
        index: token.index,
    ...
    // 中文注释：一旦出现闭合块，优先使用最后一个真实闭合块；
    // 禁止消息结尾仅用于说明的未闭合 `<tag>` 覆盖已完成产物。
    const complete = completeBlocks.at(-1);
    const open = complete?.open ?? openStack.at(-1);
    if (!open) return null;

    const attrs = parseXmlAttributes(open.attrsText);
    const isComplete = Boolean(complete);
    const value = text.slice(open.end, complete?.closeIndex ?? text.length).trim();
  ...
  const setupHandlers = () => {
    ...
    socket.value.on("connect_error", (error) => {
      ...
      // 安全中文原因回调；调用方负责去重 toast，禁止重复刷屏。
      const reason = error?.message?.includes("xhr poll error") || error?.message?.includes("websocket error")
        ? "无法连接本地 Agent 服务，请检查应用是否在运行后重试"
        : "Agent 连接失败，请点击重新连接";
      onError?.({ code: "SOCKET_CONNECT_ERROR", message: reason });
    });
  };
  ...
  const connect = () => {
    ...
    if (!socket.value) {
      socket.value = io(url, {
        // 必须与 App Engine.IO path 一致；默认 /socket.io 不会携带 Path=/api 的 tj_session。
        path: ENGINE_IO_PATH,
        ...
        timeout: 10000,
        // 同源 Cookie 由浏览器自动携带，不向脚本暴露中央 JWT。
  ...
  const disconnect = () => {
    // 取消待执行的延迟 reconnect，避免卸载后旧项目 Socket 回连
    clearReconnectTimer();
