import { STORYBOARD_SECRET_BLOCKED_CHANNELS } from "./types";

const SHOT_ONE_EXIT_STATE =
  "顾宁右手握住空白信封，铜钥匙仍在顾宁左侧外套口袋；许舟左手悬在身侧；两人同时朝走廊尽头停住";
...
function createShot(input: {
  ...
  storyFactLocks: Record<string, unknown>;
...
function createBookBindings(): Record<string, unknown>[] {
  return [
    {
      source: "story_fact",
...
function createBookRevealGuard(): Record<string, unknown> {
  return {
    source: "story_fact",
    factId: "fact_hidden_relation_04",
    notBefore: { chapter: 7, sequence: 0, label: "正式揭示点" },
    blockedChannels: [...STORYBOARD_SECRET_BLOCKED_CHANNELS],
  };
}

export function createBookStoryboardDirectorV12Fixture(): Record<string, unknown> {
...
export function createTaskStoryboardDirectorV12Fixture(): Record<string, unknown> {
