...
exports.derivePromptFromStructuredShot = derivePromptFromStructuredShot;
exports.adaptStoryboardDirectorV12ToStructuredData = adaptStoryboardDirectorV12ToStructuredData;
exports.normalizeStoryboardStructuredData = normalizeStoryboardStructuredData;
exports.deriveShotPromptsFromStructuredData = deriveShotPromptsFromStructuredData;
exports.summarizeStoryboardStructuredData = summarizeStoryboardStructuredData;
...
function asPositiveDuration(value) {
    if (typeof value !== "number" || !Number.isFinite(value) || value <= 0)
        return undefined;
    return value;
}
function asBeatRole(value) {
    const normalized = asTrimmedString(value).toLowerCase();
    if (normalized === "opening" || normalized === "escalation" || normalized === "payoff")
        return normalized;
    return undefined;
}
...
function parseShotNoFromShotId(value) {
...
function stringifyStringList(value, limit) {
    return uniqueStrings(asStringList(value, limit)).join("、");
}
function readPromptCn(value) {
...
function derivePromptFromStructuredShot(shot) {
    return asTrimmedString(shot?.render?.promptText);
}
function summarizeVisibleStoryFactLocks(locks) {
    const objectiveDirectives = [];
    const viewpointDirectives = [];
    for (const binding of locks.bindings) {
        if (binding.visibility === "hidden")
            continue;
        const directive = `${binding.category}：${binding.directive}`;
        if (binding.visibility === "objective")
            objectiveDirectives.push(directive);
        else
            viewpointDirectives.push(directive);
    }
    return [
...
function adaptStoryboardDirectorV12ToStructuredData(value) {
    const validation = (0, contract_1.validateStoryboardDirectorV12Contract)(value);
    if (!validation.ok)
        return null;
    const { globalStyle, storyFactsContext } = validation.value;
    const shots = validation.value.shots.map((validatedShot) => {
    ...
    return {
        ...
        sourceSchemaVersion: types_1.STORYBOARD_DIRECTOR_V12_SCHEMA_VERSION,
        storyFactsContext,
...
function normalizeStoryboardStructuredData(value) {
    return adaptStoryboardDirectorV12ToStructuredData(value);
}
function readStoryboardStructuredProjection(value) {
    const record = asRecord(value);
    if (record?.version !== "two_phase_v1" ||
        record.sourceSchemaVersion !== types_1.STORYBOARD_DIRECTOR_V12_SCHEMA_VERSION ||
...
function deriveShotPromptsFromStructuredData(value) {
...
function summarizeStoryboardStructuredData(value) {
    const structured = readStoryboardStructuredProjection(value) ?? normalizeStoryboardStructuredData(value);
    if (!structured)
        return "";
