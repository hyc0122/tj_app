...
exports.createEmptyShotTable = exports.SHOT_TABLE_OVERVIEW_ORDER = exports.VIDEO_EVIDENCE_SHOT_TABLE_COLUMNS = exports.DEFAULT_SHOT_TABLE_COLUMNS = void 0;
exports.DEFAULT_SHOT_TABLE_COLUMNS = [
 ...
 * 供用户明确发起的剧本转分镜创作流程使用。
 */
exports.VIDEO_EVIDENCE_SHOT_TABLE_COLUMNS = exports.DEFAULT_SHOT_TABLE_COLUMNS
    .filter((column) => column.key !== '节拍单元' && column.key !== '剧本特征')
    .map((column) => ({ ...column }));
exports.SHOT_TABLE_OVERVIEW_ORDER = ['集数/标题', '总镜数', '素材总时长', '节拍数', '全程说明'];
const createEmptyShotTable = () => {
    const columns = exports.DEFAULT_SHOT_TABLE_COLUMNS.map((column) => ({ ...column }));
    return {
        version: 1,
        overview: {
            '集数/标题': '',
        ...
        rows: [{
                id: 'shot-1-segment-1',
                shotId: 'shot-1',
                values: Object.fromEntries(columns.map((column) => [column.key, column.label === '镜号' ? 'M001' : ''])),
            }],
    };
};
exports.createEmptyShotTable = createEmptyShotTable;
