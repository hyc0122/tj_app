...
exports.normalizeShotTable = void 0;
const types_1 = require("./types");
const normalizeShotTable = (value) => {
    if (!(0, types_1.isShotTableRecord)(value))
        return { ok: false, issues: ['分镜表数据不是对象。'] };
    if (value.version !== 1)
        return { ok: false, issues: ['分镜表版本必须为 1。'] };
    ...
    value.columns.forEach((column, index) => {
        if (!(0, types_1.isShotTableRecord)(column)) {
    ...
    if (columns.length === 0)
        issues.push('分镜表至少需要一列。');
    const rowIds = new Set();
    const rows = [];
    value.rows.forEach((row, index) => {
        if (!(0, types_1.isShotTableRecord)(row) || !(0, types_1.isShotTableRecord)(row.values)) {
    ...
    if (rows.length === 0)
        issues.push('分镜表至少需要一行。');
    const overview = {};
    if (!(0, types_1.isShotTableRecord)(value.overview)) {
        issues.push('分镜表缺少 overview 对象。');
    }
    else {
        Object.entries(value.overview).forEach(([key, raw]) => {
            if (typeof raw !== 'string')
                issues.push(`总览字段“${key}”不是字符串。`);
            else
                overview[key] = raw;
        });
    }
...
exports.normalizeShotTable = normalizeShotTable;
