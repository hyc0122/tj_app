...
type ParsedShotBlock = {
  body: string
  index: number
}

const normalizeInput = (rawText: string): string => rawText.replace(/\r\n?/g, '\n').trim()
...
const parseLabeledLines = (rawText: string, context: string): ParsedFields => {
  const fields: ParsedFields = { values: {}, order: [], issues: [] }
...
const splitShotBlocks = (text: string): ShotTableParseResult | ParsedShotBlock[] => {
  const starts = Array.from(text.matchAll(/^=========单镜头开始=========\s*$/gm))
  if (starts.length === 0) {
    return { ok: false, issues: ['没有解析到完整的“=========单镜头开始=========”标记。'] }
  }
  const blocks: ParsedShotBlock[] = []
...
const parseTimelineRows = (rawText: string, shotIndex: number): ShotTableParseResult | ParsedFields[] => {
  if (!rawText.trim()) {
  ...
  for (const rawLine of rawText.split('\n')) {
    ...
    if (current) {
      activeLabel = appendField(
        current,
        label,
        match[2] ?? '',
        `第 ${shotIndex + 1} 个镜头第 ${rows.length + 1} 个时序`,
      )
    }
  }
  pushCurrent()
...
const validateExpectedColumns = (columns: readonly ShotTableColumn[]): string[] => {
  const labels = new Set<string>()
  const issues: string[] = []
  for (const column of columns) {
    if (labels.has(column.label)) issues.push(`当前分镜表存在重复列名：“${column.label}”。`)
...
const validateFieldSet = (
  fields: ParsedFields,
  expected: readonly ShotTableColumn[],
  scope: ShotTableColumnScope,
  context: string,
): string[] => {
...
const buildOutputColumns = (
  expectedColumns: readonly ShotTableColumn[] | undefined,
  shotColumns: ShotTableColumn[],
  timelineColumns: ShotTableColumn[],
): ShotTableColumn[] => {
  if (expectedColumns) return expectedColumns.map((column) => ({ ...column }))
...
const mapRowsToColumns = (rows: ShotTableRow[], columns: ShotTableColumn[]): ShotTableRow[] => rows.map((row) => ({
  ...row,
  values: Object.fromEntries(columns.map((column) => [column.key, row.values[column.label] ?? ''])),
}))

export const parseShotTableText = (
  rawText: string,
  options: ShotTableParseOptions = {},
): ShotTableParseResult => {
  const text = normalizeInput(rawText)
  if (!text) return { ok: false, issues: ['分镜原文为空。'] }
  ...
  const expectedIssues = options.expectedColumns
    ...
    : []
  if (expectedIssues.length > 0) return { ok: false, issues: expectedIssues }

  const parsedBlocks = splitShotBlocks(text)
  if (!Array.isArray(parsedBlocks)) return parsedBlocks
  ...
  if (parsedOverview.issues.length > 0) return { ok: false, issues: parsedOverview.issues }
  if (parsedOverview.order.length === 0) return { ok: false, issues: ['“【镜头总览】”没有字段。'] }

  const shotColumns: ShotTableColumn[] = []
  const timelineColumns: ShotTableColumn[] = []
  const seenColumns = new Set<string>()
  const rows: ShotTableRow[] = []
  const structuralFields: Array<{ fields: ParsedFields; scope: ShotTableColumnScope; context: string }> = []
  ...
  for (const block of parsedBlocks) {
    ...
    if (shotFields.issues.length > 0) return { ok: false, issues: shotFields.issues }
    ...
    for (let timelineIndex = 0; timelineIndex < timelineRows.length; timelineIndex += 1) {
      ...
      if (options.expectedColumns) {
        const timelineIssues = validateFieldSet(
          timeline,
          options.expectedColumns,
          'timeline',
          `第 ${block.index + 1} 个镜头第 ${timelineIndex + 1} 个时序`,
        )
        if (timelineIssues.length > 0) return { ok: false, issues: timelineIssues }
      }
