...
// 配置
const localesDir = process.env.I18N_CHECK_LOCALES_DIR || './src/locales/language';
const srcDir = process.env.I18N_CHECK_SRC_DIR || './src';
...
// 获取所有 i18n key
function getLocaleKeySets(): Map<string, Set<string>> {
  ...
  files.forEach(file => {
    ...
    locales.set(path.basename(file, '.json'), keys);
  });
...
// 主逻辑
function main(): void {
  const locales = getLocaleKeySets();
  const baselineLocale = process.env.I18N_CHECK_BASELINE_LOCALE || 'zh-CN';
  const allKeys = locales.get(baselineLocale);
  if (!allKeys) {
    console.error(`缺少 i18n 基准语言包: ${baselineLocale}`);
    process.exitCode = 1;
    return;
  }
  for (const [locale, keys] of locales) {
    const missingFromLocale = [...allKeys].filter(key => !keys.has(key));
    if (!missingFromLocale.length) continue;
    console.log(`\n⚠️  ${locale}.json 缺少基准 key (${missingFromLocale.length}):`);
    missingFromLocale.sort().forEach(key => console.log(`  - ${key}`));
    process.exitCode = 1;
  }
