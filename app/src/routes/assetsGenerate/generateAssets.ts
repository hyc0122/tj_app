...
import {
  ...
  VendorReferenceUnsupportedError,
...
export default router.post("/", validateFields(requestSchema), async (req, res) => {
  ...
  try {
    ...
    const safe = e instanceof VendorGenerationPhaseError || e instanceof VendorReferenceUnsupportedError
      ? e
      : createSafeVendorPhaseError("execute");
    await u
      .db("o_image")
      .where("id", imageId)
      .update({ state: "生成失败", errorReason: safe.message });
    return res.status(400).send(error(safe.message, { code: safe.code }, 400));
  }
});
