...
export default router.post(
  ...
  async (req, res) => {
    ...
    const data = await u.accountDb("o_vendorConfig").where("id", vendor.id).first();
    if (data) return res.status(500).send(error("供应商id已存在"));
    const privateInputs = vendor.inputValues ?? {};
    const { afterVendorConfigWrite, commitVendorConfigMutation } = await import("@/tianjiang/sync/profile-settings-adapter");
    await commitVendorConfigMutation(u.accountDb, { op: "upsert", id: String(vendor.id) }, async (trx) => {
      await trx("o_vendorConfig").insert({
        id: vendor.id,
        inputValues: JSON.stringify(privateInputs),
    ...
    u.vendor.writeCode(
      vendor.id,
      sanitizeVendorSourceSecrets(tsCode, vendor.inputs, privateInputs),
    );
    await afterVendorConfigWrite({ op: "upsert", id: String(vendor.id) });
