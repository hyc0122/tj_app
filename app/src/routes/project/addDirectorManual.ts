...
export default router.post(
  ...
  async (req, res) => {
    try {
      ...
      const safeId = assertSafeSkillSegment(directorManual, "导演手册");
      const { skillsRoot, styleDir: probe } = await resolveManualStyleDirectory(
        u.getPath(),
        "story_skills",
        safeId,
        { mustExist: false },
      ...
      const mainPath = resolveAccountSkillPath(
        skillsRoot,
        path.posix.join("story_skills", safeId),
      ...
      for (const item of data) {
        ...
        const relative = subDir
          ? path.posix.join("story_skills", safeId, subDir, `${item.value}.md`)
          : path.posix.join("story_skills", safeId, `${item.value}.md`);
