...
export default router.post(
  ...
  async (req, res) => {
    try {
      ...
      const safeStyle = assertSafeSkillSegment(stylePath, "视觉手册");
      const { styleDir: existingProbe, skillsRoot } = await resolveManualStyleDirectory(
        u.getPath(),
        "art_skills",
        safeStyle,
        { mustExist: false },
      ...
      const mainPath = resolveAccountSkillPath(
        skillsRoot,
        path.posix.join("art_skills", safeStyle),
      ...
      for (const item of data) {
        ...
        const relative = subDir
          ? path.posix.join("art_skills", safeStyle, subDir, `${item.value}.md`)
          : path.posix.join("art_skills", safeStyle, `${item.value}.md`);
        const filePath = resolveAccountSkillPath(skillsRoot, relative, {
          kind: "file",
          mustExist: false,
        });
