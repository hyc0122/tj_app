...
export default router.post(
  ...
  async (req, res) => {
    ...
    // 模型映射与自定义 Prompt 属账号配置；分镜/轨道数据仍读项目库
    const videoPrompt = await u.accountDb("o_prompt").where("type", "videoPromptGeneration").first();
    let videoPromptGeneration = "" as string | undefined;

    const { readBoundModelPromptContent } = await import("@/tianjiang/sync/profile-settings-adapter");
    videoPromptGeneration = await readBoundModelPromptContent(id, modelData) || "";
