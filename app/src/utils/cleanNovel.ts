...
class CleanNovel {
  ...
  private async processChapter(novel: o_novel): Promise<EventType | null> {
    try {
      ...
      // 自定义 Prompt 在账号库；业务写事件仍走项目库
      const { resolveAccountPromptText } = await import("@/utils/account-model-resolver");
      const eventExtraction = (await resolveAccountPromptText("eventExtraction")) ?? undefined;
