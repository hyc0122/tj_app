...
import { currentAccountSkillsRoot } from "@/tianjiang/skills/account-skills";
...
export async function useSkill(input: SkillInput) {
  ...
  const rootDir = currentAccountSkillsRoot(getPath());
...
export function createSkillTools(skills: { name: string; description: string }[], skillPaths: SkillPaths, rootDir: string = currentAccountSkillsRoot(getPath())) {
