...
import { sanitizeMediaLog } from "@/tianjiang/media/media-safety";

export interface VendorRuntimeHooks {
  provider: string;
  /** 中文注释：模板预检必须禁用一切网络原语，收费执行阶段才允许恢复。 */
  networkPolicy?: "enabled" | "blocked";
  onRemoteTaskCreated?(
    remoteTaskId: string,
    metadata: { requestPath: string },
  ): Promise<void>;
}
...
export default function runCode(
  ...
  hooks?: VendorRuntimeHooks,
) {
  code = code.replace(/export\s*\{\s*\};?/g, ""); // 去掉 export {} 以免沙盒环境报错
  // 创建一个沙盒
  const exports = {};
  let capturedRemoteTask = false;
  const captureRemoteTask = async (method: string, requestUrl: unknown, payload: unknown) => {
    ...
    capturedRemoteTask = true;
  };
  const blockNetwork = () => {
    throw new Error("供应商模板预检禁止网络访问");
  };
  const trustedFetch: typeof fetch = async (input, init) => {
    const response = await fetch(input, init);
    if (response.ok) {
      try {
        await captureRemoteTask(init?.method ?? "GET", input, await response.clone().json());
      } catch (error) {
        if (error instanceof SyntaxError) {
          // 非 JSON 同步响应不属于异步任务创建。
        } else {
          throw error;
        }
      }
  ...
  const trustedAxios = wrapAxiosForTaskCapture(axios, captureRemoteTask);
  const blockedAxios = new Proxy(blockNetwork as any, {
    apply: blockNetwork,
    get: () => blockNetwork,
  });
  const networkBlocked = hooks?.networkPolicy === "blocked";
  class BlockedFormData {
    constructor() {
      // 中文注释：form-data.submit 可直接使用 Node HTTP，预检阶段连实例都不能创建。
      blockNetwork();
    }
  }
  const sandbox: Record<string, any> = {
    // 中文注释：SDK 工厂同样具备网络能力；预检 VM 里必须同步阻断，避免模板顶层绕过 fetch/axios 门禁。
    createOpenAI: networkBlocked ? blockNetwork : createOpenAI,
    createDeepSeek: networkBlocked ? blockNetwork : createDeepSeek,
    createZhipu: networkBlocked ? blockNetwork : createZhipu,
    createQwen: networkBlocked ? blockNetwork : createQwen,
    createAnthropic: networkBlocked ? blockNetwork : createAnthropic,
    createOpenAICompatible: networkBlocked ? blockNetwork : createOpenAICompatible,
    createXai: networkBlocked ? blockNetwork : createXai,
    createMinimax: networkBlocked ? blockNetwork : createMinimax,
    createGoogleGenerativeAI: networkBlocked ? blockNetwork : createGoogleGenerativeAI,
    zipImage,
    zipImageResolution,
    urlToBase64: networkBlocked ? blockNetwork : urlToBase64,
    mergeImages,
    pollTask,
    fetch: networkBlocked ? blockNetwork : trustedFetch,
    exports,
    axios: networkBlocked ? blockedAxios : trustedAxios,
    FormData: networkBlocked ? BlockedFormData : FormData,
    logger,
    jsonwebtoken,
    crypto,
  };
...
function wrapAxiosForTaskCapture(
  source: typeof axios,
  capture: (method: string, requestUrl: unknown, payload: unknown) => Promise<void>,
): typeof axios {
  const wrapInstance = (instance: any): any => new Proxy(instance, {
    apply: async (target, thisArg, argumentList) => {
      const response = await Reflect.apply(target, thisArg, argumentList);
      await capture(
        String(argumentList[0]?.method ?? "GET"),
        argumentList[0]?.url,
    ...
    get(target, property, receiver) {
      if (property === "create") {
        return (...args: any[]) => wrapInstance(target.create(...args));
      }
      if (["post", "put", "patch", "request"].includes(String(property))) {
        return async (...args: any[]) => {
          const response = await target[property](...args);
          const method = property === "request" ? String(args[0]?.method ?? "GET") : String(property);
          const requestUrl = property === "request" ? args[0]?.url : args[0];
          await capture(method, requestUrl, response?.data);
          return response;
        };
      }
  ...
  return wrapInstance(source);
}

function safeRequestPath(input: unknown): string {
  const raw = typeof input === "string"
    ? input
    : input instanceof URL
      ? input.toString()
      : typeof (input as { url?: unknown } | undefined)?.url === "string"
...
function extractRemoteTaskId(payload: unknown, provider: string): string | undefined {
  ...
  const idProviders = new Set(["atlascloud", "grsai", "vidu", "volcengine", "volcengineSd2"]);
  const candidates = [
...
export function logger(logstring: any) {
  console.log("【VM】" + JSON.stringify(sanitizeMediaLog(logstring)));
}
/**
 * 压缩图片，目标字节数不高于 size
 */
