...
import { consumeFullStreamForExecution } from "@/agents/scriptAgent/script-agent-execution";
import { finalizeProductionDirectorPlanOutput } from "./production-agent-execution";
import {
  ProductionAgentOutputError,
  type ProductionAgentExecutionStage,
...
export interface AgentContext {
  ...
  /** 当前轮是否已提交至少一个 production artifact。 */
  artifactCommitted?: boolean;
  /** 项目事务成功后立即登记 durable mutation intent。 */
...
export async function runDecisionAI(ctx: AgentContext) {
  ...
  const skill = resolveAccountSkillFile(currentAccountSkillsRoot(u.getPath()), "production_agent_decision.md");
...
async function createSubAgent(parentCtx: AgentContext) {
  ...
  async function runAgent({
    ...
    /** 仅允许冻结高级部署键，禁止任意 `${string}:${string}` 动态键 */
    ...
    if (stage === "directorPlan") {
      const collected = await consumeFullStreamForExecution(streamResult.fullStream, subMsg, {
        deferComplete: true,
        abortSignal,
      });
      let finishReason = collected.streamFinishReason ?? null;
      try {
        const source = (streamResult as unknown as {
          finishReason?: PromiseLike<string> | string;
        }).finishReason;
        if (source != null) finishReason = await Promise.resolve(source);
      } catch {
        // 流内 finishReason 已作为后备；读取 SDK Promise 失败不伪造成功。
      }
      const finalized = await finalizeProductionDirectorPlanOutput({
        collected,
        subMsg,
        ...
        onArtifactCommitted: () => {
          parentCtx.artifactCommitted = true;
          parentCtx.onArtifactCommitted?.();
        },
      });
      fullResponse = finalized.fullResponse;
    } else {
      fullResponse = await consumeFullStream(streamResult.fullStream, subMsg);
  ...
  //     const skill = resolveAccountSkillFile(currentAccountSkillsRoot(u.getPath()), "production_agent_execution.md");
  ...
  const run_sub_agent_derive_assets = tool({
    ...
    execute: async ({ prompt }) => {
      const skill = resolveAccountSkillFile(currentAccountSkillsRoot(u.getPath()), "production_execution_derive_assets.md");
      const systemPrompt = await fs.promises.readFile(skill, "utf-8");
      return runAgent({
  ...
  const run_sub_agent_generate_assets = tool({
    ...
    inputSchema: jsonSchema<{ prompt: string }>(promptInput),
    execute: async ({ prompt }) => {
      const skill = resolveAccountSkillFile(currentAccountSkillsRoot(u.getPath()), "production_execution_generate_assets.md");
      const systemPrompt = await fs.promises.readFile(skill, "utf-8");
      return runAgent({
  ...
  const run_sub_agent_director_plan = tool({
    ...
    inputSchema: jsonSchema<{ prompt: string }>(promptInput),
    execute: async ({ prompt }) => {
      const skill = resolveAccountSkillFile(currentAccountSkillsRoot(u.getPath()), "production_execution_director_plan.md");
      const systemPrompt = await fs.promises.readFile(skill, "utf-8");
  ...
  const run_sub_agent_storyboard_gen = tool({
    ...
    inputSchema: jsonSchema<{ prompt: string }>(promptInput),
    execute: async ({ prompt }) => {
      const skill = resolveAccountSkillFile(currentAccountSkillsRoot(u.getPath()), "production_execution_storyboard_gen.md");
      const systemPrompt = await fs.promises.readFile(skill, "utf-8");
      return runAgent({
  ...
  const run_sub_agent_storyboard_panel = tool({
    ...
    inputSchema: jsonSchema<{ prompt: string }>(promptInput),
    execute: async ({ prompt }) => {
      const skill = resolveAccountSkillFile(currentAccountSkillsRoot(u.getPath()), "production_execution_storyboard_panel.md");
      const systemPrompt = await fs.promises.readFile(skill, "utf-8");
  ...
  const run_sub_agent_storyboard_table = tool({
    ...
    inputSchema: jsonSchema<{ prompt: string }>(promptInput),
    execute: async ({ prompt }) => {
      const skill = resolveAccountSkillFile(currentAccountSkillsRoot(u.getPath()), "production_execution_storyboard_table.md");
      const systemPrompt = await fs.promises.readFile(skill, "utf-8");
  ...
  const run_sub_agent_supervision = tool({
    ...
    inputSchema: jsonSchema<{ prompt: string }>(promptInput),
    execute: async ({ prompt }) => {
      const skill = resolveAccountSkillFile(currentAccountSkillsRoot(u.getPath()), "production_agent_supervision.md");
      const systemPrompt = await fs.promises.readFile(skill, "utf-8");
      return runAgent({
...
async function createArtSkills(artName: string, storyName: string) {
  ...
  const artSafe = assertSafeSkillSegment(artName, "视觉手册");
  const storySafe = assertSafeSkillSegment(storyName, "导演手册");
  const artWorkerPath = resolveAccountSkillPath(skillsRoot, path.posix.join("art_skills", artSafe, "driector_skills"), { kind: "directory", mustExist: false });
  const storyWorkerPath = resolveAccountSkillPath(skillsRoot, path.posix.join("story_skills", storySafe, "driector_skills"), { kind: "directory", mustExist: false });
  ...
  try {
    for await (const chunk of fullStream) {
      ...
      if (chunk.type === "reasoning-start") {
        ...
        // AI SDK 6：子工具失败通过 tool-error 传播，禁止继续下一次收费模型步。
    ...
    thinking?.complete();
    if (err instanceof ProductionAgentOutputError || err?.name === "ProductionAgentOutputError") {
      // 子消息已经携带安全中文与稳定错误码，禁止把供应商/栈信息拼进正文。
      throw err;
    }
    text.error();
    msg.error("Production Agent 执行失败，请重试");
    throw err;
  }
...
async function useProductionSkills(artName: string, storyName: string) {
  ...
  const artWorkerPath = resolveAccountSkillPath(skillsRoot, path.posix.join("art_skills", artSafe, "driector_skills"), { kind: "directory", mustExist: false });
  const storyWorkerPath = resolveAccountSkillPath(skillsRoot, path.posix.join("story_skills", storySafe, "driector_skills"), { kind: "directory", mustExist: false });
  const productionPath = resolveAccountSkillPath(skillsRoot, "production_skills", { kind: "directory", mustExist: false });
  const skillList = [
