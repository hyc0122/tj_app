...
class MessageBuilder {
  ...
  // 错误：稳定 errorCode/stage + 安全中文 fallback（禁止供应商堆栈/密钥）
  error(errorMsg?: string, meta?: { errorCode?: string; stage?: string }) {
    const ext: Record<string, string> = {};
    if (errorMsg) ext.error = errorMsg;
    if (meta?.errorCode) ext.errorCode = meta.errorCode;
    if (meta?.stage) ext.stage = meta.stage;
    this.socket.emit("message:update", {
      id: this.messageId,
      status: "error" as ChatMessageStatus,
