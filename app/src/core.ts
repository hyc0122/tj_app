...
export default async function generateRouter(): Promise<void> {
  ...
  const explicitlyMountedRoutes = new Set([
    "src/routes/tianjiang/control-plane.ts",
    "src/routes/tianjiang/runtime.ts",
  ]);
  // 这两个路由依赖专用生产中间件，必须只由 app.ts 显式装配一次。
  entries = entries.filter((entry) => !explicitlyMountedRoutes.has(entry.replace(/\\/g, "/")));
  // 排序
