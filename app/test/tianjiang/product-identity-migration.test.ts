...
test("产品机器标识迁移独立，后续账号迁移保持追加顺序", () => {
  ...
  assert.equal(latest?.version, 47);
  assert.equal(latest?.name, "jiasu-provider-named-materials-v5-1");
  assert.equal(migrations.find((item) => item.name === "canvas-import-staging-reservations-v1")?.version, 45);
  assert.ok(migrations.some((item) => item.name === "jiasu-provider-model-catalog-v4-4"));
