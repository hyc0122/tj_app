...
/**
 * 正式服务 bundle 会从 data/serve 的相邻目录读取内置 Skills。
 * 每次构建都从受控源码重建该目录，避免开发启动或集成测试使用陈旧基线。
 */
function stageBuiltinSkills(): void {
  const sourceRoot = path.resolve("src/tianjiang/skills/builtin");
  const sourceManifest = path.resolve("src/tianjiang/skills/builtin-skills-manifest.json");
  const targetRoot = path.resolve("data/builtin-skills");
  const targetManifest = path.resolve("data/builtin-skills-manifest.json");
...
(async () => {
  try {
    ...
    // 每次正式构建都先从受控路由目录重建 router.ts，禁止依赖开发机遗留的忽略文件。
